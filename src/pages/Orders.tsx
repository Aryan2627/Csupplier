  ...
  return (
    <div className="glass-panel" style={{ padding: '32px', minHeight: 'calc(100vh - 64px)' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        ...
        <div style={{ backgroundColor: '#eff6ff', color: '#1d4ed8', padding: '8px 16px', borderRadius: '4px', fontSize: '0.9rem', fontWeight: 600 }}>
          {orders.length} Total Orders
        </div>
      </div>

      {error ? (
        <div style={{ padding: '24px', backgroundColor: '#fef2f2', border: '1px solid #fecaca', color: '#ef4444', borderRadius: '4px' }}>
          {error}
        </div>
      ) : orders.length === 0 ? (
        <div style={{ padding: '48px 24px', textAlign: 'center', backgroundColor: '#f8fafc', border: '1px dashed #cbd5e1', borderRadius: '4px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '20px' }}>
          {orders.map(order => (
            <div key={order.id} style={{ backgroundColor: '#fff', borderRadius: '4px', padding: '24px', border: '1px solid #e2e8f0', boxShadow: 'none', cursor: 'pointer' }}  >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '16px' }}>
                ...
                <div style={{ padding: '4px 10px', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 600, backgroundColor: order.status === 'Pending Vendor' || order.status === 'Draft' ? '#fef3c7' : order.status === 'Rejected' ? '#fef2f2' : '#ecfdf5', color: order.status === 'Pending Vendor' || order.status === 'Draft' ? '#b45309' : order.status === 'Rejected' ? '#ef4444' : '#059669', border: `1px solid ${order.status === 'Pending Vendor' || order.status === 'Draft' ? '#fde68a' : order.status === 'Rejected' ? '#fca5a5' : '#a7f3d0'}` }}>
                  {order.status}
                </div>
              </div>
